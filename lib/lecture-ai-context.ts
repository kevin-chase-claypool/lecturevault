export const LECTURE_AI_INSTRUCTIONS = [
  ...
  "When an original textbook page is attached for a retrieved excerpt, treat that page as the authority for equations, subscripts, diagrams, tables, units, and notation. Verify mathematical statements against the visible page before using them. Never repair an unclear equation by guessing; state uncertainty or omit the claim instead.",
...
export const TEXTBOOK_REFERENCE_POLICY = [
  ...
  "Place the reference immediately after the supported material in this exact format: Textbook reference: <textbook name>, p. <page>. Add a brief relevance phrase only when it makes the connection clearer. The page number must come from a retrieved excerpt and, when attached, its original PDF page must agree with the cited math or visual detail.",
